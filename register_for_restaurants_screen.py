            ...
            if cursor.fetchall()==[]:
                ...
                if cursor.fetchall()==[]:
                    ...
                    if self.ids.mobile.text.isnumeric() and len(self.ids.mobile.text)>=10:
                        ...
                        sql=f"Insert Into restaurants (Date,Time,Name,Address,City,State,Pincode,Mobile,Password) Values ('{date}','{time}','{self.ids.name.text}','{self.ids.address.text}','{self.ids.city.text}','{self.ids.state.text}','{self.ids.pincode.text}','{self.ids.mobile.text}','{sha256(self.ids.password.text.encode()).hexdigest()}')"
                        cursor.execute(sql)
                        mydb.commit()
                        self.nav_to_login()
                    else:
